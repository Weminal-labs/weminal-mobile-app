                ...
                delegate: SliverChildBuilderDelegate(
                    (context1, index) => GestureDetector(
                          ...
                          onTap: () async {
                            ...
                            final burnResp =
                                await widget.client.executeMoveCall(txCall);
                            if (burnResp.confirmedLocalExecution == true) {
                              debugPrint(burnResp.digest);

                              showSnackBar(context1,
                                  "Burned token object txn ${burnResp.digest}");
    ...
    return SliverList(
      ...
      [
        ...
        if (packageId == null)
          ElevatedButton(
              onPressed: () async {
                final pulishTx = PublishTransaction(ContractBytes.nft_modules,
                    ContractBytes.nft_dependencies, 100000000);
                final resp = await widget.client.publish(pulishTx);
                if (resp.effects?.status.status ==
                    ExecutionStatusType.failure) {
                  showSnackBar(context, resp.effects?.status.error ?? "");
                  return;
                }
                final objectChanges = resp.objectChanges as List;
                final package =
                    objectChanges.firstWhere((e) => e["type"] == "published");
          ...
          ElevatedButton(
              onPressed: () async {
                ...
                if (resp.confirmedLocalExecution == true) {
                  final createdObjs =
                      resp.objectChanges?.where((e) => e["type"] == "created");
                  final nftObjects = createdObjs
                      ?.map<SuiObject>((e) => SuiObject.fromJson(e))
                      .toList();
